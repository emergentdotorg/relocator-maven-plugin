 ...
 */
@Named("changeling-lifecycle-participant")
@Singleton
@Slf4j
public class ChangelingLifecycleParticipant extends AbstractMavenLifecycleParticipant {

    public static final String APPLIED_CONTEXT_KEY = "changeling.applied";

    private final ProjectDependenciesResolver resolver;

    @Inject
    public ChangelingLifecycleParticipant(ProjectDependenciesResolver resolver) {
    ...
    public void afterProjectsRead(MavenSession session) throws MavenExecutionException {
        ...
        if (ConfigParser.isSkipped(session)) {
            log.info("changeling: dependency replacement is skipped ({})", ConfigParser.SKIP_PROPERTY);
            return;
        }
        for (MavenProject project : session.getProjects()) {
            try {
                ...
                throw new MavenExecutionException(
                        "changeling: invalid replacement configuration in " + project.getId() + ": " + e.getMessage(),
                        e);
            }
        }
    }
    ...
    private void replaceDependencies(MavenSession session, MavenProject project) {
        ...
        applied.forEach(replacement -> log.info("changeling: {} in {}", replacement, project.getArtifactId()));
    ...
    private DependencyNode collectDependencyGraph(MavenSession session, MavenProject project) {
        ...
        try {
            ...
            if (graph == null) {
                log.warn(
                        "changeling: could not collect dependency graph of {}; transitive replacement skipped: {}",
                        project.getId(), e.getMessage());
            } else {
                log.debug(
                        "changeling: dependency graph of {} collected partially: {}",
                        project.getId(),
                        e.getMessage());
            }
            return graph;
