...
Changeling (`org.emergent.maven.changeling:changeling-maven-plugin`) is a single-module Maven plugin that replaces
...
All logic lives in one package, `org.emergent.maven.changeling`. The essential mechanism spans
...
- The participant stashes applied-replacement summaries in a project context value
  (`ChangelingLifecycleParticipant.APPLIED_CONTEXT_KEY`) which `changeling:list` reads back.
...
`src/it/<name>/` is a self-contained project whose pom declares changeling-maven-plugin (version token
