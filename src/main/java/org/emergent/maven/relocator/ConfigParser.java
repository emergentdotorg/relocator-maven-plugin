...
public final class ConfigParser {
    ...
    public static final String PLUGIN_ARTIFACT_ID = "relocator-maven-plugin";
    public static final String SKIP_PROPERTY = "relocator.skip";
    public static final String SKIP_ENV = "RELOCATOR_SKIP";
    ...
    public static List<ReplacementRule> rulesFor(MavenProject project) {
        return project.getBuildPlugins().stream()
                .filter(ConfigParser::isRelocatorPlugin)
                .findFirst()
                .map(plugin -> parseRules((Xpp3Dom) plugin.getConfiguration()))
                .orElse(List.of());
    }

    private static boolean isRelocatorPlugin(Plugin plugin) {
    ...
    private static List<ReplacementRule> parseRules(Xpp3Dom configuration) {
        ...
        for (Xpp3Dom replacement : replacements.getChildren("replacement")) {
            rules.add(ReplacementRule.of(childValue(replacement, "source"), childValue(replacement, "target")));
        }
