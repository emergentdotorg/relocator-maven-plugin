...
Changeling is a Maven plugin that replaces specific dependencies with substitutes during the build.
...
<build>
  <plugins>
    <plugin>
      ...
      <artifactId>changeling-maven-plugin</artifactId>
      <version>1.0</version>
      <extensions>true</extensions>
      <configuration>
        <replacements>
          <replacement>
            <from>log4j:log4j</from>
            <to>ch.qos.reload4j:reload4j:1.2.26</to>
          </replacement>
        </replacements>
      </configuration>
    </plugin>
...
Every applied replacement is logged at info level, prefixed with `changeling:`.
...
Goal prefix: `changeling`.

- `changeling:list` — prints the configured replacement rules and, when the extension is active, the
  replacements applied to the current project.
- `changeling:help` — help for the plugin's goals.

## Disabling

Pass `-Dchangeling.skip=true` (or set the environment variable `CHANGELING_SKIP=true`) to skip replacement for a build.
